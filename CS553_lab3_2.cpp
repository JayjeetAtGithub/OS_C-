...
void handler(int sig)
{
	...
	cout<<"The number is found.Killing Processes "<<endl;
	fflush(stdout);
	kill(x,SIGUSR1);
	exit(0);
}
...
int main()
{
	...
	int n,m,l,c=0;
	//pid_t x;
	int flag; // to detect whether the child has already found
	vector<int>v;
	int index[100];
	//vector<int> sum;
	...
	cin>>n;
	FILE *f;
	f=fopen("LOG_filessearch_1.txt","w");
	random_device rd; 
	...
	cin>>p;
	f=fopen("LOG_filessearch_1.txt","a");
	int k;
	if(f)
	{
		k=ftell(f);
	...
	for(int j=0;j<n;j++)
	{
		...
		{	
			...
			ca=0;
			f1=fopen("LOG_filessearch_1.txt","r");
			if(f1)
			{
		...
		if(x!=0)
		{
			...
			//cout<<"A"<<endl;
			cb=0;
			f2=fopen("LOG_filessearch_1.txt","r");
			if(f2)
			{	
		//		cout<<"a"<<endl;
